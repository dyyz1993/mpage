...
const outputDir = path.join(__dirname, '../tests/fixtures/layouts');
...
async function main() {
  ...
  console.log(`开始提取 ${files.length} 个网站的布局结构...\n`);

  const results: Record<string, { layout: LayoutNode | null; ascii: string; htmlSize: number }> =
    {};
  ...
  console.log(`\n完成！结果保存在 ${outputDir}`);
  console.log(`- 每个网站的布局: ${outputDir}/{name}.txt`);
