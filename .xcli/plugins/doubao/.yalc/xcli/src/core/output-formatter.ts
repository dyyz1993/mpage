...
export class OutputFormatter {
  ...
  private formatYaml(data: unknown): string {
    const lines: string[] = [];
    this.toYamlLines(data, lines, 0);
    return lines.join('\n');
  }

  private toYamlLines(data: unknown, lines: string[], indent: number): void {
    ...
    if (Array.isArray(data)) {
      ...
      for (const item of data) {
        lines.push(`${prefix}-`);
        this.toYamlLines(item, lines, indent + 1);
      }
      return;
    }
    ...
    if (typeof data === 'object') {
      ...
      for (const [key, value] of entries) {
        ...
        this.toYamlLines(value, lines, indent + 1);
      }
      return;
    }
