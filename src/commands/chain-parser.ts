...
export function parseCommandChain(input: string): ParsedPipeline[] {
  ...
  for (let i = 0; i < input.length; i++) {
    const char = input[i];
    if (!inQuote && (char === '"' || char === "'")) { inQuote = char; current += char; continue; }
    if (inQuote && char === inQuote) { inQuote = null; current += char; continue; }
    if (char === "(") parenDepth++;
    if (char === ")") parenDepth--;
    ...
    if (!inQuote && parenDepth === 0) {
      ...
      if (char === ";") {
        if (current.trim()) currentPipeline.push(current.trim());
        if (currentPipeline.length > 0) result.push({ pipeline: currentPipeline, type: "sequence" });
        currentPipeline = []; current = ""; continue;
      }
    }
    current += char;
  }
