...
export const daemonBuiltin: BuiltinCommand = {
  ...
  execute: async (args, _options, _ctx) => {
    ...
    if (action === 'status' || !action) {
      ...
      console.log(`Daemon: ${status.running ? 'Running' : 'Not running'}`);
      if (status.running) {
    ...
    console.log('Usage: xcli daemon [start|stop|status]');
  },
};
...
export const sessionsBuiltin: BuiltinCommand = {
  ...
  execute: async (_args, _options, _ctx) => {
    if (!isDaemonRunning()) {
...
export const killBuiltin: BuiltinCommand = {
  name: 'kill',
  description: '杀死所有进程',
  help: {
    usage: 'xcli kill',
    description: '关闭 daemon 和所有 session',
    options: [],
    examples: [{ cmd: 'xcli kill', description: '杀死所有进程' }],
  },
  execute: async (_args, _options, _ctx) => {
    await killAllDaemon();
