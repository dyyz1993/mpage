...
import {
  ...
  CONFIG_KEY_MAP,
  CONFIG_FILE,
} from '../core/rc-config.js';
import {
  listGuardRules,
  addGuardRule,
  removeGuardRule,
  setGuardIdentityKey,
} from '../core/agent-guard.js';
...
function handleGuard(args: string[]): void {
  ...
  if (action === 'add') {
    ...
    addGuardRule(identity, { match, block, message });
  ...
  if (action === 'remove') {
    ...
    const ok = removeGuardRule(identity);
    if (!ok) {
