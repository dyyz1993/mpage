import { checkSession } from '../core/session-client';
...
export async function checkCommand(args: string[], values: Record<string, any>) {
  ...
  try {
    const result = await checkSession(session, selector);
