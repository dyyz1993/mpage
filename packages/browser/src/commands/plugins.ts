...
export async function pluginsCommand(args: string[], values: CommandValues, core?: Core) {
  ...
  if (action === 'list' || action === 'ls') {
    ...
    const loadedPlugins = core?.loader.getLoadedPlugins() ?? [];
  ...
  if (action === 'reload') {
    ...
    try {
      const instance = core?.loader ? await core.loader.reloadPlugin(name) : undefined;
      if (instance) {
        console.log(`Plugin "${name}" reloaded (status: ${instance.status})`);
      } else {
        console.error('No core loader available');
      }
    } catch (err) {
      console.error(
...
async function doctorCommand(core?: Core) {
  ...
  for (const plugin of plugins) {
    ...
    const instance = core?.loader.getPlugin(plugin.name);
