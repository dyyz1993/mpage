...
const core = new Core({
  name: 'xcli',
  version: '1.0.0',
  description: 'Extensible browser automation CLI',
  configDirName: '.xcli',
  envPrefix: 'XCLI',
  pluginDirs: ['.xcli/plugins', '../.xcli/plugins', '~/.xcli/plugins'],
...
async function main() {
  ...
  const cdpValue = values.cdp;
  if (cdpValue !== undefined && cdpValue !== null && cdpValue !== true) {
    const port = String(cdpValue);
    if (/^\d+$/.test(port)) {
      try {
  ...
  const { checkGuard } = await import('@xcli/core');
  const guardResult = checkGuard(core, cmd);
  ...
  if (isBuiltinCommand) {
    try {
      const { executeBuiltin } = await import('@xcli/browser');
      await executeBuiltin(cmd, cmdArgs, values, core);
    } catch {
  ...
  if (site) {
    ...
    if (siteCmdObj) {
      const { executeSiteCommand } = await import('@xcli/browser');
