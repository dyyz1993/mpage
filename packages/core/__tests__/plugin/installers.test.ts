...
describe('NpmInstaller', () => {
  ...
  it('install: constructs correct npm install command', async () => {
    mockExec.mockReturnValue('');
    const nmDir = join(tempDir, 'node_modules', 'test-pkg');
    ...
    const result = await installer.install('test-pkg');

    expect(mockExec).toHaveBeenCalledWith(
      expect.stringContaining('npm install test-pkg'),
      expect.objectContaining({ stdio: 'pipe' })
    );
    expect(result.id).toBe('npm:test-pkg');
    expect(result.type).toBe('npm');
  });

  it('install: includes version in command when specified', async () => {
    mockExec.mockReturnValue('');
    const nmDir = join(tempDir, 'node_modules', 'ver-pkg');
    mkdirSync(nmDir, { recursive: true });
    writeFileSync(
      join(nmDir, 'package.json'),
      JSON.stringify({ name: 'ver-pkg', version: '2.0.0' })
    );

    const installer = new NpmInstaller(tempDir);
    await installer.install('ver-pkg', { version: '2.0.0' });

    expect(mockExec).toHaveBeenCalledWith(
      expect.stringContaining('ver-pkg@2.0.0'),
      expect.any(Object)
    );
  });

  it('install: includes registry flag when specified', async () => {
    mockExec.mockReturnValue('');
    const nmDir = join(tempDir, 'node_modules', 'reg-pkg');
    mkdirSync(nmDir, { recursive: true });
    writeFileSync(
      join(nmDir, 'package.json'),
      JSON.stringify({ name: 'reg-pkg', version: '1.0.0' })
    );

    const installer = new NpmInstaller(tempDir);
    await installer.install('reg-pkg', { registry: 'https://registry.example.com' });
  ...
  it('install: throws on npm command failure', async () => {
    const err = new Error('npm failed') as Error & { stderr: string };
    ...
    const installer = new NpmInstaller(tempDir);
    await expect(installer.install('fail-pkg')).rejects.toThrow('Failed to install fail-pkg');
  });

  it('install: handles scoped packages (@org/plugin-name)', async () => {
    mockExec.mockReturnValue('');
    const scopedDir = join(tempDir, 'node_modules', '@myorg', 'scoped-pkg');
    mkdirSync(scopedDir, { recursive: true });
    writeFileSync(
      join(scopedDir, 'package.json'),
      JSON.stringify({ name: '@myorg/scoped-pkg', version: '1.0.0' })
    );

    const installer = new NpmInstaller(tempDir);
    const result = await installer.install('@myorg/scoped-pkg');

    expect(result.id).toBe('npm:@myorg/scoped-pkg');
    expect(result.name).toBe('@myorg/scoped-pkg');
    expect(result.path).toBe(scopedDir);
  });

  it('install: throws when package.json not found after install', async () => {
    mockExec.mockReturnValue('');

    const installer = new NpmInstaller(tempDir);
    mkdirSync(join(tempDir, 'node_modules'), { recursive: true });

    await expect(installer.install('ghost-pkg')).rejects.toThrow('package.json not found');
  });

  it('uninstall: constructs correct npm uninstall command', async () => {
    mockExec.mockReturnValue('');

    const installer = new NpmInstaller(tempDir);
    await installer.uninstall('npm:some-pkg');

    expect(mockExec).toHaveBeenCalledWith(
      expect.stringContaining('npm uninstall some-pkg'),
      expect.objectContaining({ stdio: 'pipe' })
    );
  });

  it('update: runs npm update and re-installs', async () => {
    mockExec.mockReturnValue('');
    const nmDir = join(tempDir, 'node_modules', 'upd-pkg');
    mkdirSync(nmDir, { recursive: true });
    writeFileSync(
      join(nmDir, 'package.json'),
      JSON.stringify({ name: 'upd-pkg', version: '1.5.0' })
    );
    ...
    expect(mockExec).toHaveBeenCalledWith(
      expect.stringContaining('npm update upd-pkg'),
      expect.any(Object)
    );
    expect(result.version).toBe('1.5.0');
  });

  it('list: returns empty when node_modules does not exist', async () => {
    const installer = new NpmInstaller(tempDir);
    const list = await installer.list();
    expect(list).toStrictEqual([]);
  });

  it('list: scans node_modules for installed packages', async () => {
    mockExec.mockReturnValue('');

    const nmDir = join(tempDir, 'node_modules');
    const pkgDir = join(nmDir, 'listed-pkg');
    mkdirSync(pkgDir, { recursive: true });
    writeFileSync(
      join(pkgDir, 'package.json'),
      JSON.stringify({ name: 'listed-pkg', version: '1.0.0' })
    );
    ...
    expect(list).toHaveLength(1);
    expect(list[0].name).toBe('listed-pkg');
  });
});
